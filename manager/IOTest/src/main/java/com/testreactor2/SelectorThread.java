...
//                System.out.println(Thread.currentThread().getName() + "   :  after select...." + selector.keys().size());
                ...
                System.out.println(Thread.currentThread().getName() + "   :  select....nums=>" + nums);
    ...
    private void readHander(SelectionKey key) throws IOException {
        ...
        while (true) {
            try {
                ...
                if (num > 0) {
                    ...
                    buffer.get(bytes);
                    System.out.println("client: " + client.getRemoteAddress() + "send date " + new String(bytes));
                    buffer.flip();
                ...
                System.out.println("client: " + client.getRemoteAddress() + "远程主机强迫关闭了");
                key.cancel();
                break;
                // e.printStackTrace();
            }
